...
def get_disease_model():
    ...
    if disease_model is None:
        ...
        # 1. Build the MobileNetV2 base structure
        ...
        # 2. Add the classification head to your model
...
async def analyze_plant(file: UploadFile = File(...), scan_type: ScanType = Form(...)):
    ...
    if scan_type.value == "leaf":
        ...
        # If YOLO didn't find any fruits, return an error immediately
        if fruit_count == 0:
            return {"status": "Failed", "error": "No tomatoes detected in the image."}
        
        # If tomatoes are found, check if they are ripe
